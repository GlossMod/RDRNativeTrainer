...
void DrawText(float x, float y, const char *str, int r, int g, int b, int a)
{
	...
	// 绘制文本
	drawText(x, y, str, r, g, b, a, FontID::Redemption, 0.06f, TextJustification::Left); // 中心位置，白色，字体ID 0，字体大小0.5，居中对齐
}
...
void MenuItemBase::OnDraw(float lineTop, float lineLeft, bool active)
{
	...
	DrawText(lineLeft + m_textLeft, lineTop + m_lineHeight / 4.5f, const_cast<char *>(GetCaption().c_str()), color.r, color.g, color.b, color.a);
...
void MenuItemSwitchable::OnDraw(float lineTop, float lineLeft, bool active)
{
	...
	// GUI::GUI_SET_TEXT_COLOR(color.r, color.g, color.b, static_cast<int>(color.a / 1.1f));

	DrawText(lineLeft + lineWidth - lineWidth / 6.35f, lineTop + lineHeight / 4.8f, GetState() ? "[开]" : "[关]", color.r, color.g, color.b, color.a);
}
...
void MenuItemValue::OnDraw(float lineTop, float lineLeft, bool active)
{
	...
	DrawText(lineLeft + lineWidth - lineWidth / 5.0f, lineTop + lineHeight / 4.8f, (char *)CVal3, color.r, color.g, color.b, color.a / 1.1f);
}
...
void MenuItemMenu::OnDraw(float lineTop, float lineLeft, bool active)
{
	...
	DrawText(lineLeft + lineWidth - lineWidth / 8, lineTop + lineHeight / 3.5f, ">>", color.r, color.g, color.b, color.a / 2);
}
...
void MenuController::DrawStatusText()
{
	if (GetTickCount() < m_statusTextMaxTicks)
	{
		...
		DrawText(0.5, 0.5, const_cast<char *>(m_statusText.c_str()), 255, 255, 255, 255);
	}
}
